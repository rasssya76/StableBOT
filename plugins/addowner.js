...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	if (global.own.map(([number]) => number).includes(who.split('@')[0])) return m.reply('[ ! ] Dia sudah jadi owner.')
	if (global.owner.map(([number]) => number).includes(who.split('@')[0])) return m.reply('[ ! ] Dia sudah jadi real owner.')
	global.own.push([who.split('@')[0], text[0], true])
