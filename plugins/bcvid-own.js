...
let handler = async (m, { conn, text, participants  }) => {     
  ...
  let yah = await uploadImage(media)
  ...
  let inipesan = `──•「 VIDEO BROADCAST 」•──`+`\n\n`+text
  // console.log(msg)
    for (let i of anu) {
    await conn.sendFile(i, yah, inipesan, wm, ftroli, null, { fileLength: fsizedoc, seconds: `90000000`})
   //await conn.relayMessage(i, msg.message, { messageId: msg.key.id })
    }
  m.reply(`Sukses Mengirim Broadcast Ke ${anu.length} Group`)
}
handler.command = ['bcvideo']
handler.owner = true
