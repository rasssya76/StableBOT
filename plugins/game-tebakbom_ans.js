...
handler.before = async function (m, {
    ...
    if (!(id in conn.tebakbom) && m.quoted && /❏  *B O M B*/i.test(m.quoted.text)) return conn.reply(m.chat, `Sesi telah berakhir, silahkan kirim .tebakbom untuk membuat sesi baru.`, m)
            ...
            if (open >= 8) {
                ...
                await conn.sendMessage(m.chat, {
                    ...
                    contextInfo: {
                        externalAdReply: {
                            title: ``,
                            thumbnailUrl: `https://telegra.ph/file/f6ebfea2758b947e1e49d.jpg`,
                            mediaType: 1,
                            renderLargerThumbnail: true
                        }
                    }
                ...
                db.data.users[m.sender].money += reward
                clearTimeout(conn.tebakbom[id][2])
                delete conn.tebakbom[id]
            } else {
                let teks = `❏  *B O M B*\n\n`
                ...
                conn.reply(m.chat, teks, m).then(() => {
                    if (user) db.data.users[m.sender].money += reward
                })
            }
        }
    }
