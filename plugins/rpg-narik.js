...
let handler = async (m, { conn, command, args }) => {
  ...
  if (global.db.data.users[m.sender].bank >= xpperlimit * count) {
    ...
    conn.reply(m.chat, `Sukses menarik sebesar ${toSimpl(count)} Money 💹`, m)
  } else conn.reply(m.chat, `[❗] Uang dibank anda tidak mencukupi untuk ditarik sebesar ${toSimpl(count)} money 💹`, m)
}
