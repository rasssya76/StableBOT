...
`.trim()
            ...
            if (mythic > 0) {
                   ...
                   conn.replyV2(m.chat, '*Selamat anda mendapatkan item Rare yaitu*\n' + mythic + `${rpg.emoticon('mythic')}Mythic Crate`, m)
            }
            if (legendary > 0) {
        ...
        } else conn.replyV2(m.chat, `Minimal 80 health untuk bisa bercocok tanam, beli obat dulu biar kuat dengan ketik *${usedPrefix}shop buy potion <jumlah>*\ndan ketik *${usedPrefix}use potion <jumlah>*\n\n_Untuk mendapat money dan potion gratis ketik_ *${usedPrefix}claim*\nHealing ${usedPrefix}heal\nBeli Potion ${usedPrefix}shop buy potion`, m)
    } catch (e) {
        console.log(e)
        throw eror
    }
}
handler.help = ['petualang', 'work', 'dungeon']
handler.tags = ['rpg']
handler.command = /^(petualang|work|dungeon)$/i
